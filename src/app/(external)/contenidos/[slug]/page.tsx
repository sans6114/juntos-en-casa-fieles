...
export default async function ContenidoPage({ params }: ContenidoPageProps) {
  ...
  // Zod exige `descripcion` no vacia al guardar, pero una fila vieja o escrita
  // fuera del panel puede traer "". Sin contenido, el bloque entero se va.
        ...
        <section className="campo-papel px-6 pb-24 md:px-10 md:pb-28 lg:px-16">
          <div className="mx-auto grid max-w-6xl items-start gap-10 lg:grid-cols-[minmax(0,1fr)_368px]">
            <div className="flex min-w-0 flex-col gap-10">
              ...
              {/* La regla superior separa la descripcion del video. Sin video
                  arriba no separa nada: seria una linea suelta bajo el titulo. */}
              {descripcion ? (
                <div
                  className={
                    item.youtubeId ? "border-t-[3px] border-[var(--regla)] pt-10" : undefined
                  }
                >
                  ...
                  <p className="max-w-2xl text-pretty text-base leading-[1.75] text-[var(--suave)] md:text-lg">
                    <BrandName>{descripcion}</BrandName>
                  </p>
                </div>
              ) : null}
            </div>
