...
/**
 * Prerenderiza en el build la página de cada producto publicado, mirror de
 * `contenidos/[slug]/page.tsx:38-41` (commit `792d1b6`). `dynamicParams`
 * queda en su default (`true`): un slug publicado después del build se
 * renderiza on-demand, y `crearProducto`/`toggleProductoPublicado` lo
 * revalidan. Devuelve `[]` con el catálogo vacío — mismo comportamiento que
 * el flag `PRODUCTOS_RESUELTO = false` de antes.
 */
...
export async function generateMetadata({ params }: ProductoPageProps): Promise<Metadata> {
  ...
  const item = await obtenerProductoPorSlug(slug)
...
export default async function ProductoPage({ params }: ProductoPageProps) {
  ...
  const relacionados = await obtenerProductosRelacionados(item.slug)
